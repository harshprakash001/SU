...
// ✅ Redirect 2: Astrology Website Index → Results Page
app.get('/index2.html', (req, res) => {
    res.redirect('/results.html');
});
...
app.get('/api/panchang', async (req, res) => {
    ...
    try {
        ...
        $('.dpTableRow').each((_, row) => {
            ...
            const key = keyElement.text().trim();
            const value = valueElement.contents().map((_, el) => $(el).text().trim()).get().filter(text => text).join(' ');
...
app.post('/api/match-making', async (req, res) => {
    try {
        const response = await axios.post('https://json.apiastro.com/match-making/ashtakoot-score', req.body, {
            headers: { 'Content-Type': 'application/json', 'x-api-key': 'skU7HNl9tG40AzU5L8vqV27uXIfxhp4B7TXwFk1q' }
        });
        res.json(response.data);
    } catch (error) {
        console.error('Error in match-making:', error.message);
...
// ✅ Required for Vercel: Export app instead of app.listen()
module.exports = app;
